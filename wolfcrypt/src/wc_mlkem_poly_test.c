...
/* Number of bytes in an output block of Ascon-XOF128 */
#define ASCON_XOF_BYTES  16
/* Number of bytes in an output block of Ascon-Hash256 */
#define ASCON_HASH_BYTES 32

/* Number of blocks to generate for matrix using Ascon-XOF. */
#define GEN_MATRIX_NBLOCKS \
    ((12 * MLKEM_N / 8 * (1 << 12) / MLKEM_Q + ASCON_XOF_BYTES) / ASCON_XOF_BYTES)
/* Number of bytes to generate for matrix. */
#define GEN_MATRIX_SIZE     GEN_MATRIX_NBLOCKS * ASCON_XOF_BYTES
 ...
 *
 * FIPS 203, Section 4.1 (Modified for Ascon):
 * 1. XOF.init() = AsconXOF128.Init().
 * 2. XOF.Absorb(ctx,str) = AsconXOF128.Absorb(ctx,str).
 *
 * @param  [in, out]  asconXof  Ascon-XOF128 object.
 * @param  [in]       seed      Data to absorb.
 * @param  [in]       len       Length of data to absorb in bytes.
 * @return  0 on success always.
 */
static int mlkem_xof_absorb(wc_AsconXof128* asconXof, byte* seed, int len)
{
    int ret;

    ret = wc_AsconXof128_Init(asconXof);
    if (ret == 0) {
        ret = wc_AsconXof128_Absorb(asconXof, seed, len);
    }
 ...
 *
 * FIPS 203, Section 4.1 (Modified for Ascon):
 * 3. XOF.Squeeze(ctx,l) = AsconXOF128.Squeeze(ctx,8.l).
 *
 * @param  [in, out]  asconXof  Ascon-XOF128 object.
 ...
 * @return  0 on success always.
 */
static int mlkem_xof_squeezeblocks(wc_AsconXof128* asconXof, byte* out, int blocks)
{
    return wc_AsconXof128_Squeeze(asconXof, out, blocks * ASCON_XOF_BYTES);
}
#endif

/* New/Initialize Ascon-Hash256 object.
 *
 * FIPS 203, Section 4.1 (Modified for Ascon):
 * H(s) := AsconHash256(s)
 *
 * @param  [in, out]  hash    Ascon-Hash256 object.
 * @param  [in]       heap    Dynamic memory allocator hint.
 * @param  [in]       devId   Device id.
 * @return  0 on success always.
 */
int mlkem_hash_new(wc_AsconHash256* hash, void* heap, int devId)
{
    (void)heap;
    (void)devId;
    return wc_AsconHash256_Init(hash);
}

/* Free Ascon-Hash256 object.
 *
 * FIPS 203, Section 4.1 (Modified for Ascon):
 * H(s) := AsconHash256(s)
 *
 * @param  [in, out]  hash  Ascon-Hash256 object.
 */
void mlkem_hash_free(wc_AsconHash256* hash)
{
 ...
 * @return  0 on success.
 */
int mlkem_hash512(wc_AsconXof128* prf, const byte* data1, word32 data1Len,
                  const byte* data2, word32 data2Len, byte* out)
{
    int ret;

    /* MLKemKey의 prf 컨텍스트를 해시 목적으로 재초기화하며 사용 */
    ret = wc_AsconXof128_Init(prf);
    if (ret == 0) {
        ret = wc_AsconXof128_Absorb(prf, data1, data1Len);
    }
    /* Check if there is a second block of data. */
    if ((ret == 0) && (data2Len > 0)) {
        ret = wc_AsconXof128_Absorb(prf, data2, data2Len);
    }
    if (ret == 0) {
        /* Generate 64 bytes of output (equivalent to SHA3-512) */
        ret = wc_AsconXof128_Squeeze(prf, out, 64);
    }
 ...
 */
void mlkem_prf_init(wc_AsconXof128* prf)
{
    /* * Ascon-XOF 연산을 새로 시작하기 위해
     * 전체 초기화 함수를 호출하여 상태를 리셋합니다.
     */
    wc_AsconXof128_Init(prf);
}
 ...
 * @return  0 on success always.
 */
int mlkem_prf_new(wc_AsconXof128* prf, void* heap, int devId) {
    (void)heap; (void)devId; /* Ascon Init doesn't use these */
    return wc_AsconXof128_Init(prf);
}
 ...
 * @return  0 on success always.
 */
static int mlkem_prf(wc_AsconXof128* prf, byte* out, unsigned int outLen,
                     const byte* key)
{
    int ret;
    ...
    ret = wc_AsconXof128_Init(prf);
    if (ret == 0) {
        /* Process all data including the index byte */
        ret = wc_AsconXof128_Absorb(prf, key, WC_ML_KEM_SYM_SZ + 1);
    }
    if (ret == 0) {
        /* Generate the required amount of output */
